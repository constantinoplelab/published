% builds the null distribution of trial shuffles for the regression
...
nshuff = 20; %number of times to do the shuffle
    ...
    for m = 1:nfold
        for j = 1:ntx
            ...
            try
                ...
                coeftest_model(n,m,j) = MnrMdl.coefTest;
                Rsquared_model(n,m,j) = MnrMdl.Rsquared.Ordinary;
    
                preds_CV(n,m,j,:) = MnrMdl.predict(Xtest_mj);
                true_CV(n,m,j,:) = Ytest_mj;
    
                preds_train(n,m,j,:) = MnrMdl.predict(Xtrain_mj);
                true_trian(n,m,j,:) = Ytrain_mj;
    
                pvals(n,m,j,:) = MnrMdl.Coefficients{:,'pValue'};
                coeffs_model(n,m,j,:) = MnrMdl.Coefficients{:,'Value'};
...
if usemlb
    savename = strcat(savedir,'decode_mlb_alldat_dprime_shuffle_',dprimetype,'_quantile.mat');
else
    savename = strcat(savedir,'decode_second_mlb_alldat_dprime_shuffle_',dprimetype,'_quantile.mat');
end
