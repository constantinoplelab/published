function [WTOpt, WTMdl, BlkInf, Belief, Prior, Kappa] =...
    GenerateSynthData_Bayes_Bias(params, ratTrial,...
    noise_arg, gen_arg, noise_params)
%GenerateSynthData_Bayes - generates synthetic wait times
%for optimal Bayes model.
% Inputs:
%   params - Parameter vector. [kappa_mi, kappa_hi, kappa_lo, D, lambda, bias]
...
p_b = [1-H0-bias,   H0+bias,   H0-bias;...    % mix
       (H0/2)-bias, 1-H0+bias, 0-bias;...     % high
       (H0/2)-bias  0+bias,    1-H0-bias];    % low
...
for rr = 1:length(rew_idx)
    if ismember(rr, NewSessIdx)
        ...
        % %add in bias term
        if rand<=0.3
            % belief(1,rr) = belief(1,rr);
        belief(1,rr) = belief(1,rr)-bias;
        if belief(1,rr)<0
            belief(1,rr)=0;
        end
        belief(2,rr) = belief(2,rr)+bias;
        % belief(3,rr) = belief(3,rr);
        belief(3,rr) = belief(3,rr)-bias;
        if belief(3,rr)<0
            belief(3,rr)=0;
        end
        end
        ...
        %normalize belief to being between 0:1
