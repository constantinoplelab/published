...
% Check screen perf unless you just want to look at one date or don't want
% to screen
if isempty(varargin)
...
elseif length(varargin) > 1  %don't use screening
    goods = find(cellfun(@(s) sum(s.TrainingStage==9)==length(s.TrainingStage), ...
        S.pd)); %added by SS 02/1/2024
end
...
S2.pd = S.pd;
S2.peh = S.peh;
ITI = getITI(structfun(@(s) s(include_sess), S2, 'UniformOutput', false));
data.ITI = cell2mat(ITI');

slrt = getSLRT(structfun(@(s) s(include_sess), S2, 'UniformOutput', false));
...
blockTransI = [0;...
    find(diff(data.block) ~= 0 | diff(data.trial_num) < 0);...
    length(data.trial_num)];

[block_num, block_position] = deal(cell(1, length(blockTransI)-1));

for ii = 1:length(blockTransI)-1
    block_num{ii} = 1:blockTransI(ii+1)-blockTransI(ii);
    
    n = max(block_num{ii});
    i1 = floor(n/2);
    i2 = round(rem(n/2, 1));
    
    block_position{ii} = [1:i1, -1*(i1+i2:-1:1)];
end
data.block_position = cell2mat(block_position)';
