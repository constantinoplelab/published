function plotFigureS5_muscimolModelsSupp(behaviorPath)
% Plot figure S5. Simulate other versions of the inference model.
...
p_optEarly = signrank(simControl_lambda.mtol(:,12), simControl_lambda.mtoh(:,13));
p_optLate = signrank(simControl_lambda.mtol(:,16), simControl_lambda.mtoh(:,16));
p_lambdaEarly = signrank(simMuscimol_lambda.mtol(:,12), simMuscimol_lambda.mtoh(:,13));
...
p_optKEarly = signrank(simControl_kappa.mtol(:,13), simControl_kappa.mtoh(:,13));
p_optKLate = signrank(simControl_kappa.mtol(:,16), simControl_kappa.mtoh(:,16));
p_kappaEarly = signrank(simMuscimol_kappa.mtol(:,13), simMuscimol_kappa.mtoh(:,13));
...
params = [0.25 0.30 0.20 0.13];
[~, ~, ~, Belief, ~, Kappa] = GenerateSynthData_Bayes_SS(params, A_model, ...
    'logn', 1, 8, 'log');
...
% posterior belief shown in panel A
...
% inference model with lambda parameter schematic
nexttile

%changing opportunity cost schematic
nexttile
plot(t, value, 'k', 'linewidth', 1.5)
yline(0.2, 'r', 'linewidth', 1)
yline(0.15, 'color', [1 0.6 0.6], 'linewidth', 1)
yline(0.08, 'b', 'linewidth', 1)
yline(0.12, 'color', [0.6 0.6 1], 'linewidth', 1)
ylim([0 0.35])
yticks([])
xticks([])
box off
