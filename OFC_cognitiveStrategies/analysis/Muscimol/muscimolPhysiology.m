...
%Analyze firing rates pre and post muscimol infusion. Rat was anesthetized
%for the entire recording. Bilateral infusions and recordings. 

% INPUTS: 
%   muscimolPhysDataPath = path to .mat file with recording data and
%       spreadsheet with infusion info

% OUTPUTS:
%   out = struct with binned firing rate ratios
%       (post-infusion/pre-infusion) and sigmoid fit


sem = @(x) std(x,'omitnan')./sqrt(sum(any(~isnan(x), 2)));

SU_L = load([muscimolPhysDataPath, filesep, 'M011_L.mat']); %left hemisphere recording
...
[finalParams, MSE, initCond, fitParams] = my_fit_sigmoid(out.x, y, 50);
