function info = plotTCA_ephysFig(K, drawLines, varargin)
...
% modified from tensor-demo repo from Alex Williams
% SSS 5-10-2022
%%
nd = ndims(K); % Order
...
params.addParameter('Hspace',0.06); % Horizontal space between axes
params.addParameter('Hspaceright',0.025); % Horizontal space on left
params.addParameter('Hspaceleft',0.06); % Horizontal space on right
params.addParameter('Vspace',0.03); % Vertical space between axes
...
for k = 1 : nd
    ...
    zeroidx = find(sum(U,2) < 0.0001 );
    U(zeroidx, :) = []; %don't plot neurons with neuron factor = 0 for all components. SSS

    xl = [0 size(K,k)+1];
%     yl = [min( 0, min(U(:)) ), max( 0, max(U(:)) )];
    yl = [min( 0, min(U(:)) ), max( 0, max(U(:)) ) + 0.005];
...
%         set(FactorAxes(k,j),'Ytick',[]);
        ...
        h(k,j) = hh;
%         set(FactorAxes(k,j),'FontSize',7)
        set(FactorAxes(k,j), 'box', 'off')
        set(FactorAxes(k,j), 'tickdir','out')
        set(FactorAxes(k,j),'Ytick',[]);
        FactorAxes(k,j).YAxis.Visible = 'off';
                
        if k == 2
            for ii = 1:length(drawLines)
            plot(FactorAxes(k,j), [drawLines(1,ii) drawLines(1,ii)], yl, ...
                '--k', 'Linewidth', 0.75); %SSS
            plot(FactorAxes(k,j), [drawLines(2,ii) drawLines(2,ii)], yl, ...
                'color', [0.65 0.65 0.65], 'Linewidth', .75); %SSS
            end
        end
    end
end
