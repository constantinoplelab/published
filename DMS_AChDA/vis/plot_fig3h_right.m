function plot_fig3h_right(datadir)
% Plot dopamine AUC at the reward cue (0-0.5 s) vs -delta trial initiation
...
fname = fullfile(datadir, 'data-published/deltaITI_vs_AUC_SOFF.mat');
if exist(fname, 'file')
    ...
    for rr=1:length(ratList)
        ...
        [auc{rr}, bins{rr}] = deltaITI_vs_AUC_SOFF(datadir, ratname);
    end
    auc = cell2mat(auc);
...
function [auc_mean, edges] = deltaITI_vs_AUC_SOFF(datadir, rat)
% SideOff AUC on trial N vs -(ITI(N+1)-ITI(N)) = ITI(N)- ITI(N+1)
...
if iscell(bstruct.(rat).CPIn.UniqueDay(1))
    bstruct.(rat).CPIn.UniqueDay = ...
        str2double(bstruct.(rat).CPIn.UniqueDay);
end

soff_trials = [bdata.TrialNumber, bdata.UniqueDay];
% get ITI on post side-off trials from CPIn bdata
iti_postsoff = nan(length(soff_trials),1);
for tr=1:length(soff_trials)
    ix = find(bstruct.(rat).CPIn.TrialNumber == soff_trials(tr,1)+1 & ...
    bstruct.(rat).CPIn.UniqueDay == soff_trials(tr,2));
    if ~isempty(ix)
        iti_postsoff(tr) = bstruct.(rat).CPIn.ITI(ix);
    end
end
...
% determine iti cutoffs using side-off and post side-off trials
cutoff = [prctile([bdata.ITI;iti_postsoff],10), ...
    prctile([bdata.ITI;iti_postsoff],90)];
fprintf('ITI cutoffs (s): %.4f, %.4f\n', cutoff)
bdata.ITI(bdata.ITI<min(cutoff) | bdata.ITI>max(cutoff)) = nan;
iti_postsoff(iti_postsoff<min(cutoff) | iti_postsoff>max(cutoff)) = nan;
...
for bb=1:nbin
    DA_bc = baselineCorrect(T, -0.1, 0, pdata(diti_bins==bb,:));
