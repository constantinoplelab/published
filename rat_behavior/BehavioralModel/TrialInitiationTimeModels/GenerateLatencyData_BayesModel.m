function [LatOpt, LatMdl, BlkInf, RPE, Belief, Kappa] =...
    GenerateLatencyData_BayesModel(params, ratTrial,...
...
% kappa_mi = mean(log([5 10 20 40 80]))*C*log2(5)/tau;
% kappa_hi = mean(log([20 40 80]))*C*log2(5)/tau;
% kappa_lo = mean(log([5 10 20]))*C*log2(5)/tau;
% D = params(1);
...
if gen_arg
    LatMdl = generate_wts(LatOpt, 5, noise_arg, noise_params);
else
    LatMdl = nan;
end
