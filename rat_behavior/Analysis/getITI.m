...
%     st = NaN(numTrials+1,1); %start time of trial relative to start of session
%     st(1) = 0; %start of session = time 0
    I = NaN(numTrials+1,1);
    ...
    %             st(t) = st(t-1) + S.peh{s}(t-1).Events.Tup(end); %cumulative time in session
                flag = 0;
    ...
    % use if start time matters
%     if flag == 1
%         break %exclude session if timing info is incomplete
%     end
...
%     startTime{s} = st;
        ...
        %get trials in each 60 second bin (based on start time of trial)
%             t1 = arrayfun(@(x) find(tStart >= x), bins, 'UniformOutput', false);
%             t2 = arrayfun(@(x) find(tStart < (x+60)), bins, 'UniformOutput', false);
%             tInBins = arrayfun(@(x) intersect(t1{x},t2{x}), 1:length(bins), ...
%                 'UniformOutput', false);
% 
%             itiBinned = [itiBinned; cellfun(@(x) iti(x), ...
%                 tInBins, 'UniformOutput', false)]; 
