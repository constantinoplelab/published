function [ordering,groupbreaks] = time2peakByCluster(X,ops)
...
nn = numel(ops.grouping);
[~,I_order] = sort(ops.grouping);
groupbreaks = find(diff(sort(ops.grouping))); %when does a new group start
ordering = zeros(nn,1);
